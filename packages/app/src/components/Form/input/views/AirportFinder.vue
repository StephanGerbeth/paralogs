<template>
  <v-card>
    ...
    <base-collection-radio-list v-model="model" :items="markers">
      <template #default="{ item }">
        <v-list-item :title="item.tags.name" :subtitle="item.id" />
      </template>
    </base-collection-radio-list>
  </v-card>
</template>

<script setup>
import MarkerAirports from '@/components/Map/markers/Airports';
import BaseCollectionRadioList from '@/components/base/collections/RadioList';
